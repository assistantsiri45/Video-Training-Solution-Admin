...
@section('content_header')
    <h1 class="m-0 text-dark">Edit Chapter</h1>
@stop
    ...
    <div class="row">
        <div class="col-md-8">
            <div class="card card-primary">
                <form role="form" id="create" method="POST" action="{{ route('chapters.update', $chapter->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        <div class="row">
                                ...
                                <div class="form-group">
                                    ...
                                    <x-inputs.course id="course_id" class="{{ $errors->has('course_id') ? ' is-invalid' : '' }}">
                                        @if(!empty(old('course_id', $chapter->course_id)))
                                            <option value="{{ old('course_id', $chapter->course_id) }}" selected>{{ old('course_id_text', $chapter->course->name) }}</option>
                                        @endif
                                    </x-inputs.course>
                            ...
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <x-inputs.level id="level_id" related="#course_id">
                                        @if(!empty(old('level_id', $chapter->level_id)))
                                            <option value="{{ old('level_id', $chapter->level_id) }}" selected>{{ old('level_id_text', $chapter->level->name) }}</option>
                                        @endif
                                    </x-inputs.level>
                            ...
                            <div class="col-sm-6">
                                ...
                                <label>Type</label>
                                    <select class="form-control" id="package_type" name="package_type">
                                        ...
                                        @if($chapter->package_type_id == $type->packagetype->id))
                                            <option value="{{ old('package_type_id', $chapter->package_type->id) }}" selected>{{ old('level_id_text', $chapter->package_type->name) }}</option>
                                        @else
                            ...
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="name" class="form-control" id="name" placeholder="Chapter Name"
                                           @error('name') is-invalid @enderror" value="{{ old('name', $chapter->name) }}">
                                    @error('name')
...
{{--                                <input type="file" multiple accept="application/pdf" name="study_materials[]"  class="form-control" id="study_materials" @error('study_materials[]') is-invalid @enderror value="{{ old('study_materials[]') }}">--}}
...
@section('js')
    {!! $html->scripts() !!}
    <script>
         var package_type='{{$chapter->package_type_id}}';
        var level_id='{{$chapter->level_id}}';
        var subject_id='{{$chapter->subject_id}}';
