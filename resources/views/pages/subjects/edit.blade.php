    ...
    <div class="row">
        <div class="col-md-8">
            <div class="card card-primary">
                <form role="form" id="create" method="POST" action="{{ route('subjects.update', $subject->id) }}">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        <div class="row">
                                ...
                                <div class="form-group">
                                    ...
                                    <x-inputs.course id="course_id" class="{{ $errors->has('course_id') ? ' is-invalid' : '' }}">
                                        @if(!empty(old('course_id', $subject->course_id)))
                                            <option value="{{ old('course_id', $subject->course_id) }}" selected>{{ old('course_id_text', $subject->course->name) }}</option>
                                        @endif
                                    </x-inputs.course>
                            ...
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <x-inputs.level id="level_id" related="#course_id">
                                        @if(!empty(old('level_id', $subject->level_id)))
                                            <option value="{{ old('level_id', $subject->level_id) }}" selected>{{ old('level_id_text', $subject->level->name) }}</option>
                                        @endif
                                    </x-inputs.level>
                            ...
                            <div class="col-sm-6">
                                <div class="form-group"> 
                                    ...
                                    <option value="">Choose Type</option>
                                        ...
                                        @if($subject->package_type_id == $type->packagetype->id))
                                            <option value="{{ old('package_type_id', $subject->package_type->id) }}" selected>{{ old('level_id_text', $subject->package_type->name) }}</option>
                                        @endif
                                        @endif
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="name" class="form-control" id="name" placeholder="Subject Name"
                                           @error('name') is-invalid @enderror" value="{{ old('name', $subject->name) }}">
                                    @error('name')
