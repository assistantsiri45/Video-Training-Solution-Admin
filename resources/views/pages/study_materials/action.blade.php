...
<style>
    ...
    <div class="float-right">
        <a href="{{ route('study-materials.edit', $id) }}" class=""><i class="fas fa-edit ml-3"></i></a>
        @if($has_package==true)
        @if($is_enabled==true)
    ...
    <script>
        (function($){
            $("#delete-{{ $id }}").click(function () {
                ...
                if (confirmation) {
                    $.ajax({
                        url: "{{ route('study-materials.destroy', $id) }}",
                        type: "DELETE",
                        data: {
                            _token: "{{ csrf_token() }}"
                        },
                        success: function(result) {
                            if (result) {
                                toastr.success('Study material status updated');
